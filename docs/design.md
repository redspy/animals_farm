...
미확정. 다만 **폰트는 확정**이다: 웹에는 시스템 폰트 폴백이 없어 한글이 두부(□)로 나오므로 `assets/fonts/NotoSansKR-Regular.otf`(OFL)를 임베드해 `gui/theme/custom_font`로 지정한다. 폰트를 교체하더라도 한글 글리프를 포함한 임베드 폰트여야 한다.

나머지는 미확정. 현재 프로토타입은 도형(원/사각형)만으로 배치와 스케일을 검증하는 그레이박스 단계다.
